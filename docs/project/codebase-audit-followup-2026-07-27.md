...
后续分支 `codex/upgrade-cocos-2.4.15` 已继续关闭本报告原列出的工程内中长期事项：

1. **引擎维护版升级完成。** 工程、Web、Android JSB 和资源 importer 已迁移到 Creator
   2.4.15，模拟器日志确认实际运行版本。构建输入使用官方包和固定 SHA-256。
2. **大控制器继续拆分。** 近场候选和交互选择抽为 `GameplayInteractionQuery`，存档装载、
   自动保存和坏档回退抽为 `GameplayPersistence`；原控制器保留编排职责，契约测试覆盖边界。
3. **历史配置债务可审计。** 440 条警告不再只有数量上限，而是保存排序后的精确集合与
   SHA-256；新增、删除或内容变化都会要求显式审阅并更新基线。
4. **生产业务包裁剪。** 已从构建结果中移除未使用的 `RoadLayer` 模块，并增加生产包前后
   结构校验；Web Mobile 主业务包减少 14.3%。
5. **原生兼容和离线边界。** 修复 2.4.15 V8 Inspector 启动崩溃、旧音量字符串的原生
   参数错误、Android A/D/DPAD 键码不一致；下载器使用零网络离线兼容桩。

完整迁移矩阵和产物证据见
[`cocos-creator-2.4.15-migration-and-validation-2026-07-27.md`](cocos-creator-2.4.15-migration-and-validation-2026-07-27.md)。

## 仍需外部发布验收

工程内剩余工作已经完成。以下两项需要项目外设备或密钥，无法由当前源码仓库代替：

1. 至少两台真实 ARM64 设备（Adreno/Mali）完成 30 分钟七图、后台、锁屏和低内存矩阵。
2. 使用正式生产 keystore 重新构建，并验证正式证书覆盖安装；QA 测试签名不能作为生产包。
