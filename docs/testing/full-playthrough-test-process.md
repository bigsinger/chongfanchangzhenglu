...
3. 第三章结束时出现“后续关卡正在开发中，敬请期待”，确认后返回主菜单。
...
| 第三章 | 白石渡主路 | `scenes_d3_1` | 蓝布包任务、参军换装、申请物资、章节结算 |
| 第三章 | 医疗点 | `scenes_d3_2` | 两轮共九次物资分发、送错提示、补给箱交付 |
| 第三章 | 小学堂兑换点 | `scenes_d3_3` | 揭露小偷、线索答题、领取物资箱 |
...
1. 在 `scenes_d3_1` 推进到哭泣大妈，记录蓝布包线索：
   ...
   - 一张五元苏币；
   - 三张一元苏币。
2. 经 `scenes_d3_2` 到 `scenes_d3_3`，与小偷和兑换员交互。
3. 三题答案依次为：`四枚`、`一张`、`三张`。
4. 故意错一题，确认只重试当前题；再答对并完成揭露小偷剧情。
5. 返回 `scenes_d3_1`，确认大妈、二虎和王守仓后续对白解锁。
6. 完成参军换装；跨图后角色保持新服装。
...
- 当前主线完成不要求收藏满格。
- 第三节配置声明五件收藏，但七张已发布地图中只有四件可达：
  `红军八角帽`、`中革军委颁发的红军编制表`、`医疗箱`、`二角硬币`。
- `红星报`存在于物品配置和缺失提示中，但没有放置在已发布地图。若版本承诺“全收藏”，
  这是发布阻断项；若只承诺“主线通关”，应在版本说明中明确。
...
- 构建：commit / versionName / versionCode / APK SHA-256
...
- Git commit 和工作区状态；
- APK/AAB 文件名、版本号、大小、SHA-256；
- `npm test` 输出；
- 七图检查表；
- 关键检查点名称；
- `smoke`、`background`、`corrupt`、`stability` 结果；
- 终局 OCR/压缩预览；
- 真实 ARM64 设备记录；
- 未关闭问题及放行人。
...
- `npm test` 通过：67 个脚本及全部 JSON、配置图 0 错误、游戏合同、命名和纹理预算通过；
- 雷电模拟器 1 分钟短循环完成 7 个循环，依次覆盖七张已发布地图；
- 短循环中的启动、ADB 输入、Home/恢复、截图、内存记录和错误日志检查通过；
...
该短循环用于验证流程和七图矩阵本身，不替代完整人工通关，也不替代真实 ARM64 设备
30 分钟稳定性门禁。
