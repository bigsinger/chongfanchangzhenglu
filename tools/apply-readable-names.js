...
/*
 * One-time, manifest-driven naming migration for the restored Creator project.
 *
 * The APK used terse legacy module names and abbreviated resource paths. This
 * migration keeps every file-level Cocos UUID intact while moving scripts to
 * descriptive PascalCase names, moving resources to domain-oriented folders,
 * splitting chapter art into Asset Bundles, and normalizing standalone image
 * and audio filenames. Runtime aliases are generated for legacy config/save
...
function applyMigration() {
  ...
  // Move files first so parent-folder distribution (the three chapter
  // bundles) is deterministic and independent of filesystem rename order.
  ...
  // Normalize UUID-only recovered standalone image names without changing
  // their provenance directories or serialized UUID references.
  ...
  // Remove only directories made empty by the explicit moves above.
