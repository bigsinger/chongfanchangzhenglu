...
const mainDir = path.join(root, 'build', 'jsb-link', 'assets', 'main');
...
const modulePattern = /^\s{0,2}([A-Za-z_$][\w$]*): \[ function\([^)]*\) \{/gm;
const modules = [...source.matchAll(modulePattern)].map((match) => ({ name: match[1], start: match.index }));
let removedBytes = 0;
for (let index = modules.length - 1; index >= 0; index--) {
  const module = modules[index];
  if (!remove.has(module.name)) continue;
  const end = index + 1 < modules.length ? modules[index + 1].start : source.lastIndexOf('}, {}, [');
...
if (entryStart < 0 || entryArrayStart < 0 || entryArrayEnd < 0) {
...
const serializedEntries = `[ ${entries.map((name) => JSON.stringify(name)).join(', ')} ]`;
source = `${source.slice(0, entryArrayStart)}${serializedEntries}${source.slice(entryArrayEnd + 1)}`;

for (const name of remove) {
  if (new RegExp(`cc\\._RF\\.push\\([^\\n]+, "${name}"\\)`).test(source)) {
